...
class QuillLanguage(object):
    ...
    def __init__(self, langDefFile=None, langKnowledgeInput=None):
        self.specialRules = []
        self.preProcs = []
    ...
    def strToUnicode(self, str_list, options_at=None):
        ...
        if len(self.inputEncodeMap) == 0:
            ...
            literal = self.literalPreProc(''.join(str_list).lower())
            encoded_str_list = list(literal)
        else:
            encoded_str_list = [self.inputEncodeMap[x] for x in str_list]
            literal = ''.join(encoded_str_list)

        literal = self.literalPreProc(literal.lower())

        if literal in self.langDict:
            return self.langDict[literal]

        unicode_literal_chain = []
        focus = 0
        for code in encoded_str_list:
            if options_at == focus:
                unicode_literal_chain.append(self.cCart.GetClass(literal, focus, 'predictive', False))
            else:
                unicode_literal_chain.append(self.cCart.GetClass(literal, focus, 'predictive', True))
            focus += len(code)

        unicode_literal_list = [[]]
        for options in unicode_literal_chain:
            new_list = []
            for eachOption in options:
                temp = copy.deepcopy(unicode_literal_list)
                for x in temp:
                    x.append(eachOption[0])
                new_list.extend(temp)
            unicode_literal_list = new_list

        unicode_char_list_of_lists = [self.removeEpsilons(unicode_single_literal) for unicode_single_literal in unicode_literal_list]
        valid_output = []
        for char_list in unicode_char_list_of_lists:
            if self.isValidWord("".join(char_list)):
                valid_output.append(char_list)
        return valid_output
