...
class QuillEngXliterator(object):
    ...
    def __init__(self, knowledgeDir1, knowledgeDir2, xlitDef):
        ...
        if knowledgeDir1 != None and knowledgeDir2 != None:
            self.lit2engEngine = QuillLanguage.QuillLanguage(langKnowledgeInput=knowledgeDir1)
            self.eng2indEngine = QuillLanguage.QuillLanguage(langKnowledgeInput=knowledgeDir2)
        else:
            self.lit2indEngine = QuillLanguage.QuillLanguage(langKnowledgeInput=knowledgeDir1)
    ...
    def loadXlitRules(self, xlitDef):
        ...
        primaryDef = xlitRoot.attrib['primary'].split('_')[0].lower()
        primaryDef = data.language_new[primaryDef]
    ...
    def getIndianPronunciations(self, literal):
        if self.directMode == True:
            (indPronunciations, count) = self.lit2indEngine.strToUnicode(literal)
            ...
            engPronunciation = self.lit2engEngine.strToUnicode(literal)
            engPronunciation = engPronunciation[0]
            ...
            indPronunciation = self.eng2indEngine.strToUnicode(indPronounceInput)
            indPronunciation = indPronunciation[0]
            count = len(indPronunciation)
